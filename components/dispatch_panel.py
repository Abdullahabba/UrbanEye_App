...
# Safe imports with fallback definitions
try:
...
def render_dashboard_page():
    ...
    # INSTANT RESET ON INPUT MODE OR VIEW SWITCH (Clears counts, images, and tracking ID)
    current_input_mode = st.session_state.get("input_source_mode", "🖼️ Single Image")
    last_input_mode = st.session_state.get("_prev_input_mode", current_input_mode)

    if current_input_mode != last_input_mode:
        st.session_state["counts"] = {}
        st.session_state.pop("processed_img", None)
        st.session_state["captured_images"] = []
        st.session_state.pop("current_tracking_id", None)  # Clears tracking ID so a fresh one is generated
        st.session_state["stream_active"] = False
        st.session_state["_prev_input_mode"] = current_input_mode
        st.rerun()
