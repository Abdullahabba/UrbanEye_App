    ...
    with btn3:
        if st.button("💾 Submit & Log to Tracker", use_container_width=True, key="btn_save_ledger"):
            ...
            new_row = pd.DataFrame([{
                ...
                "Latitude": 31.5204, "Longitude": 74.3587, "Location Name": manual_loc_name, "Timestamp": pd.Timestamp.now().strftime("%Y-%m-%d %H:%M")
            }])
            st.session_state["incident_ledger"] = pd.concat([st.session_state["incident_ledger"], new_row], ignore_index=True)
